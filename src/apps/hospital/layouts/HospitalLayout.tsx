...
const NAV = [
  ...
  { label: "Report Symptom", href: "/hospital/ward/report-symptom", icon: "📋" },
  { label: "My Animals", href: "/hospital/ward/my-animals", icon: "🐄" },
  { label: "Alerts", href: "/hospital/ward/alerts", icon: "🔔" },
  { label: "Vaccination Schedule", href: "/hospital/ward/vaccination-schedule", icon: "💉" },
];
            ...
            <nav className="space-y-1">
              {NAV.map(n => (
                <Link
                  key={n.href}
                  to={n.href}
                  className={`sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium font-body ${location.pathname === n.href ? "active bg-[#1B4332]/10 text-[#1B4332]" : "text-gray-600 hover:text-[#1B4332]"}`}
                >
